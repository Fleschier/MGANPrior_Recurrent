...
# 使用绝对路径
import GAN.Model_Settings as Model_Settings
...
class BaseGenerator(object):
  ...
  def __init__(self, model_name, logger=None):
    ...
    for key, val in Model_Settings.MODEL_POOL[model_name].items():
      ...
      # 读取Model_Settings里面对应model的所有参数
      setattr(self, key, val)
    self.use_cuda = Model_Settings.USE_CUDA and torch.cuda.is_available() # 是否使用GPU
    self.batch_size = Model_Settings.MAX_IMAGES_ON_DEVICE   # batch大小, 一次处理的数量
    self.ram_size = Model_Settings.MAX_IMAGES_ON_RAM  # 内存最大阈值
    self.net = None
    ...
    assert self.image_channels in [1, 3]    # 图像只能是灰度图(1)或者RGB图(3)
    ...
    self.build()    # Base类不能被实例化, 需要由子类继承并复写
    # 加载pytorch预训练好的模型
    if os.path.isfile(self.weight_path):
      self.load()
    # 加载Tensorflow预训练好的模型并转化为pytorch张量 (需要复写)
  ...
  def get_value(self, tensor):
    ...
    """
    dtype = type(tensor)
    # 如果是ndarray类型的数据则直接返回
    if isinstance(tensor, np.ndarray):
      return tensor
    # 如果是torch张量则转化为numpy.ndarray
    if isinstance(tensor, torch.Tensor):
      return tensor.to(self.cpu_device).detach().numpy()    # 转化为numpy矩阵, 需要确保数据在CPU上, GPU上的数据只能是tensor
  ...
  def batch_run(self, inputs, run_fn):
    ...
    for key, val in results.items():
      # concatenate((a1, a2, …), axis=0) 数组拼接, 默认axis=0即同轴向拼接
      # 由于val可能是由之前append产生的ndarray的数组,所以这里将数组中的所有ndarray拼接到一起
  ...
  # 通过给定的latent codes来生成图像 *************************
  # 输入的latent codes需要预处理
  # 需子类复写
  ...
  def postprocess(self, images):
    ...
    # 将矩阵值域映射到[0, 255],即RGB图像亮度值, 以便生成图像
    images = (images - self.min_val) * 255 / (self.max_val - self.min_val)  # 这一步处理完后仍然可能存在非[0,255]范围内的值,由下一步处理
